...
To summarise, the E-step of the EM algorithm in case of HMM lies in estimation of $\gamma_t(i)$ which is simply a probability of being at state $i$ given our observation sequence and model parameters. Certainly, the iterative method of EM algorithm also needs an initial guess about the model parameters, meaning the initial distribution, transition and emission matrix. Since the hidden states and time are both assumed discrete we may express transition matrix using a stochastic (Markov) matrix introduced in Chapter 1. 

\[Include the initial paramaters\]

Note that the initial estimate of the parameters will never guarantee that we attain a global maximum, therefore the optimal solution would be to initialize the parameters multiple times and compare the maxima of likelihood functions. Once we have a formal estimate of the initial parameters, the EM algorithm performs E-step as defined above and subsequently M-step. The M-step

...
case of complete data. EM algorithm introduce further below is mainly based on the work of \cite{Dempster1977} and \cite{McLachlan2008}.
...
obtain a marginal probability by summing over all possible hidden states $i \in I$, as per \cite{Jurafsky2008} and \cite{Devavrat2014}:
...
i.e.\ until convergence criterion, in \cite{McLachlan2008}, is satisfied:
...
Since the log-likelihood function in Equation \ref{eq:loglike} is not analytically tractable, according to \cite{Bishop2006}, and involves logarithm of a sum,
...
artificial weights and with the use of Jensen's inequality, as suggested by \cite{Gu2008}:
...
It is also asymmetric and non-negative, and it is zero if and only if the two probability measures are equal which is a result of \textit{Gibb's inequality}. \cite{Csiszar1975}
...
\cite{Bishop2006} states to set the distribution $q(x)$ equal to the posterior distribution $p(\textbf{X}|\textbf{Y},\theta)$.
...
probability distribution is $p$.\cite{Balesdent2016} Substantially, the measure is a difference of cross-entropy denoted by $H(p,q)$ and entropy by $H(p)$ which 
...
& =  \ell(\theta|\textbf{Y}) - D_{KL}(q(X)||p(X|\textbf{Y},\theta))
\end{align}
...
with already estimated $q^{(k+1)}$, that maximizes the lower bound $L(\theta,q|\textbf{Y})$. The goal of M-step is according to \cite{Dempster1977} and \cite{Gu2008}:
...
computation described above, we take a use of \textbf{Forward} and \textbf{Backward} pass to address \textbf{E-step} of the EM algorithm, as shown by \cite{Bishop2006} and \cite{Rabiner1989}.
...
Moreover, due to Markov memoryless property, $X_t$ depends only on $X_{t-1}$. Let us now define the forward probability variable $\alpha_t(i)$ as in \cite{Bishop2006} and \cite{Rabiner1989}:
...
which is a significant improvement from the brute force approach with complexity of $O(2TN^T)$. \cite{Rabiner1989}
...
while also accounting for the emission probabilities at previous step. Effectively, there are 3 main steps, as pointed by \cite{Oliver2013}:
...
Those will result in estimating the posterior distribution of hidden states given our emission sequence \textbf{Y}, i.e. $\mathbb{P}(X_t=i|\textbf{Y},\theta)$. \cite{Rabiner1989}
...
\begin{equation}
    Y_k \sim \mathcal{N}_p(\mu_k, \Sigma_k) \quad \forall k \in \{1,\ldots,N\} 
\end{equation}
